...
function turtleScriptToJavaScript(code) {
  ...
  code = code.replace(
    /(\s|;)repeat(\s*)(\(.*\)(\s*)){/g,
    function (match, prefix, beforespaces, expr) {
      var count = gensym();
      return (
        prefix +
        "$rep$=" +
...
function fd(distance) {
  ...
  _turtle.query.x +=
    distance * _turtle.query.scale * cos(_turtle.query.heading);
  _turtle.query.y +=
    distance * _turtle.query.scale * sin(_turtle.query.heading);
...
function rt(angle, radius) {
  _ch_checkArgs(
    arguments,
    1,
    "rt(angle, <radius>) requires at least one argument",
  );
  lt(-angle, radius);
}
function lt(angle, radius) {
  _ch_checkArgs(
    arguments,
    1,
    "lt(angle, <radius>) requires at least one argument",
  );
  angle *= Math.PI / 180;
  if (radius === undefined) {
    ...
    var x2 =
      ...
      _turtle.query.scale *
        radius *
        sin(_turtle.query.heading + angle) *
        sign(angle);
    var y2 =
      y -
      _turtle.query.scale *
        radius *
        cos(_turtle.query.heading + angle) *
        sign(angle);
...
function setColor(r, g, b, a) {
  ...
  if (arguments.length === 1) {
    ...
    if (arguments.length < 3 || arguments.length > 4) {
      _ch_error(
        "Use setColor(colorname), setColor(r, g, b), or setColor(r, g, b, a)",
      );
    }
...
function startFill(r, g, b, a) {
  ...
  if (arguments.length === 1) {
    ...
    if (arguments.length < 3 || arguments.length > 4) {
      _ch_error(
        "Use startFill(colorname), startFill(r, g, b), or startFill(r, g, b, a)",
      );
    }
    c = makeColor(r, g, b, a || 1);
  }
...
var _turtle = Object.seal({
  ...
  onerror: function (message, source, line, column, error) {
    if (_ch_inInclude) {
      ...
      var text =
        error +
        " at line " +
        (parent && parent.LINE_NUMBER_OFFSET
          ? line - parent.LINE_NUMBER_OFFSET
          : line) +
        ", column " +
        column;
      var sourceLine = (
      ...
      msg.innerHTML =
        ...
        "\n" +
        new Array(Math.max(column - 1, 0)).join(" ") +
        "\u21e7</pre>" +
        text;
  ...
  updatePosition: function () {
    ...
    _turtle.image.style.top =
      Math.floor(0.5 + screenHeight / 2 - _turtle.y) + "px";
    _turtle.image.style.left =
      Math.floor(0.5 + screenWidth / 2 + _turtle.x) + "px";
    _turtle.image.style.transform =
      "rotate(" + (Math.PI / 2 - _turtle.heading) + "rad)";
  },
});
var WHITE = "#FFFFFF";
var RED = "#FF0000";
...
function onSetup() {
  ...
  _turtle.image.style.marginLeft =
    Math.floor(-_turtle.image.width / 2 + 0.5) + "px";
  _turtle.image.style.marginTop =
    Math.floor(-_turtle.image.height / 2 + 0.5) + "px";
  ...
  if (_turtle.nextCommandIndex < _turtle.commandBuffer.length) {
    ...
    if (_turtle.speed >= 1 || _turtle.frameCounter * _turtle.speed >= 1) {
      ...
      var num = Math.max(
        ...
        Math.min(
          _turtle.speed,
          _turtle.commandBuffer.length - _turtle.nextCommandIndex,
        ),
      );
...
document.write(
  '<script>_turtle.image = loadImage("https://casual-effects.com/codeheart/turtle/turtle.png"); window.onerror = _turtle.onerror;</script>',
);
